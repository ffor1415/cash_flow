...
const AddTransactionSheet = ({ open, onOpenChange, onSaved, editTransaction }: Props) => {
  ...
  return (
    <Sheet open={open} onOpenChange={v => { if (!v) reset(); onOpenChange(v); }}>
      <SheetContent
        side="bottom"
        className="bg-card border-border rounded-t-3xl overflow-y-auto"
        style={{ maxHeight: '92dvh', paddingBottom: 'max(env(safe-area-inset-bottom,0px), 1.5rem)' }}
      >
        <SheetHeader>
        ...
        <div className="space-y-4 mt-4">
          ...
          <div>
            ...
            <div className="relative mt-1.5">
              ...
              <Input
                ...
                className="bg-secondary border-border pl-9 text-2xl font-bold h-14 tracking-tight"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <Input
              placeholder={type === 'expense' ? 'e.g. Swiggy, Uber...' : 'e.g. Monthly salary...'}
              value={title} onChange={e => setTitle(e.target.value)}
              className="bg-secondary border-border mt-1.5 h-11"
            />
          </div>

          {/* Category grid — responsive cols */}
          <div>
            ...
            <div className="grid gap-2 mt-2" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(72px, 1fr))' }}>
              {visibleCats.map(cat => (
                <button key={cat.name} onClick={() => setCategory(cat.name)}
                  ...
                  <span className="text-[9px] text-muted-foreground font-medium leading-tight text-center break-words w-full">{cat.name}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {amount && category && title && (
            <div className={`rounded-xl p-3 flex items-center gap-3 ${
              type === 'expense' ? 'bg-destructive/10 border border-destructive/20' : 'bg-success/10 border border-success/20'
            }`}>
              <span className="text-2xl flex-shrink-0">{CATEGORIES.find(c => c.name === category)?.emoji}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-foreground truncate">{title}</p>
                <p className="text-xs text-muted-foreground">{category} · {date}</p>
              </div>
              <p className={`text-base font-bold flex-shrink-0 ${type === 'expense' ? 'text-destructive' : 'text-success'}`}>
