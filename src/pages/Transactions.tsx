...
const Transactions = () => {
  ...
  const handleEdit = (tx: Transaction) => { setEditTransaction(tx); setShowAdd(true); };
  const handleSheetOpenChange = (open: boolean) => { setShowAdd(open); if (!open) setEditTransaction(null); };
  ...
  const filtered = transactions.filter(tx => {
    ...
    if (monthFilter !== 'All') {
      if (new Date(tx.date).getMonth() !== parseInt(monthFilter)) return false;
    }
    return true;
  });
  ...
  return (
    <div className="bg-background page-pb w-full overflow-x-hidden">
      ...
      {/* ── Header ── */}
      <div className="page-px pt-6 pb-3">
        <div className="flex items-center justify-between mb-4 gap-2">
          <h1 className="text-xl font-heading font-bold text-foreground">Transactions</h1>
          <button
            onClick={() => { setEditTransaction(null); setShowAdd(true); }}
            className="w-10 h-10 gradient-primary rounded-xl flex items-center justify-center shadow-md flex-shrink-0 active:scale-95 transition-transform"
        ...
        <div className="flex gap-2 mb-3">
          <div className="relative flex-1 min-w-0">
            ...
            <Input
              ...
              className="bg-secondary border-border pl-9 pr-8 w-full"
            />
            {search && (
              <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 touch-min flex items-center justify-center">
          ...
          <button
            ...
            className={`relative w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 border transition-colors ${
              activeFilterCount > 0 ? 'gradient-primary border-transparent' : 'bg-secondary border-border'
            }`}
          >
        ...
        {/* Expanded filters */}
        {showFilters && (
          <div className="bg-secondary/50 rounded-2xl p-3 space-y-3 mb-3 border border-border">
            <div>
            ...
            <div>
              ...
              <div className="flex gap-1.5 flex-wrap">
                <button onClick={() => setMonthFilter('All')}
                  className={`px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all ${monthFilter === 'All' ? 'gradient-primary text-primary-foreground' : 'bg-secondary text-muted-foreground border border-border'}`}>
                  All
                </button>
                {MONTHS.map((m, i) => (
                  <button key={m} onClick={() => setMonthFilter(String(i))}
                    className={`px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all ${monthFilter === String(i) ? 'gradient-primary text-primary-foreground' : 'bg-secondary text-muted-foreground border border-border'}`}>
                    {m}
                  </button>
                ))}
              </div>
              ...
              <div>
                ...
                <div className="flex gap-1.5 flex-wrap">
                  <button onClick={() => setCategoryFilter('All')}
                    className={`px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all ${categoryFilter === 'All' ? 'gradient-primary text-primary-foreground' : 'bg-secondary text-muted-foreground border border-border'}`}>
                    All
                  </button>
                  {categories.map(c => (
                    <button key={c} onClick={() => setCategoryFilter(c)}
                      className={`px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all ${categoryFilter === c ? 'gradient-primary text-primary-foreground' : 'bg-secondary text-muted-foreground border border-border'}`}>
                      {c}
                    </button>
                  ))}
                </div>
              ...
              return (
                <div key={dateKey}>
                  ...
                  <div className="space-y-1.5">
                    {dayTx.map(tx => (
                      <div key={tx.id} className="glass-card p-3 flex items-center gap-3 group">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                            <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-medium ${
                              tx.type === 'income' ? 'bg-success/15 text-success' : 'bg-secondary text-muted-foreground'
                            }`}>
                              {tx.category}
                            </span>
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          ...
                          {/* Edit — always visible on mobile (no hover) */}
                          <button
                            onClick={() => handleEdit(tx)}
                            className="w-8 h-8 rounded-lg text-muted-foreground flex items-center justify-center active:bg-primary/10 transition-colors"
                          >
                            <Pencil className="w-3.5 h-3.5" />
                          </button>
                          <button
                            onClick={() => handleDelete(tx.id, tx.title)}
                            className="w-8 h-8 rounded-lg text-muted-foreground flex items-center justify-center active:bg-destructive/10 transition-colors"
                          >
