...
const Messages = () => {
  ...
  if (activeChat) {
    ...
    return (
      <div className="fixed inset-0 z-[60] flex flex-col bg-background" onClick={() => setShowReactions(null)}>
        <div className="flex items-center gap-3 page-px pt-[max(env(safe-area-inset-top),16px)] pb-3 bg-card border-b border-border flex-shrink-0 shadow-sm">
        ...
        {isGroup && showGroupInfo && (
          <div className="bg-card border-b border-border page-px py-3 flex-shrink-0">
              ...
              {(activeChat.members || []).map(m => {
                ...
                return (
                  <div key={m.user_id} className="flex flex-col items-center gap-1 flex-shrink-0">
                    <div className={`w-10 h-10 touch-min rounded-full flex items-center justify-center text-xs font-bold ${isCreator ? 'bg-yellow-500/20 text-yellow-500' : 'bg-primary/10 text-primary'}`}>
                  ...
                  {msgs.map((m, idx) => {
                    ...
                    return (
                      <div key={m.id} className={`flex gap-2 ${isMine ? 'justify-end' : 'justify-start'} ${showTail ? 'mt-3' : 'mt-0.5'}`}>
                        ...
                        <div className={`flex flex-col max-w-[85%] ${isMine ? 'items-end' : 'items-start'} relative group`}>
        ...
        {replyTo && (
          <div className="flex items-center gap-3 page-px py-2.5 bg-secondary border-t border-border flex-shrink-0">
        ...
        <div className="flex-shrink-0 px-3 pt-3 pb-[max(env(safe-area-inset-bottom),16px)] bg-card border-t border-border">
          <div className="flex items-center gap-2">
            <div className="flex-1 flex items-center bg-secondary rounded-2xl page-px py-2.5 min-h-[44px]">
        ...
        <Sheet open={!!msgOptions} onOpenChange={(open) => !open && setMsgOptions(null)}>
          <SheetContent side="bottom" className="bg-card border-border rounded-t-3xl pb-10">
            ...
            <div className="flex flex-col gap-2">
              <button onClick={() => { setReplyTo(msgOptions); setMsgOptions(null); inputRef.current?.focus(); }} className="w-full text-left page-px py-3 rounded-xl hover:bg-secondary font-medium transition-colors flex items-center gap-3">
                <Reply className="w-5 h-5 text-primary" /> Reply
              </button>
              <button onClick={() => { setShowReactions(msgOptions?.id || null); setMsgOptions(null); }} className="w-full text-left page-px py-3 rounded-xl hover:bg-secondary font-medium transition-colors flex items-center gap-3">
                <Smile className="w-5 h-5 text-emerald-500" /> React
              </button>
              <button onClick={handleCopy} className="w-full text-left page-px py-3 rounded-xl hover:bg-secondary font-medium transition-colors flex items-center gap-3">
              ...
              {msgOptions?.sender_id === user?.id && (
                <>
                  ...
                  <button onClick={() => handleDelete('me')} className="w-full text-left page-px py-3 rounded-xl hover:bg-destructive/10 text-destructive font-medium transition-colors flex items-center gap-3">
                    <Trash2 className="w-5 h-5" /> Delete for me
                  </button>
                  <button onClick={() => handleDelete('everyone')} className="w-full text-left page-px py-3 rounded-xl hover:bg-destructive/10 text-destructive font-medium transition-colors flex items-center gap-3">
  ...
  return (
    <div className="bg-background w-full overflow-x-hidden page-pb">
      <div className="page-px pt-6 pb-4 bg-card border-b border-border">
      ...
      <Sheet open={showNewGroup} onOpenChange={setShowNewGroup}>
        <SheetContent side="bottom" className="bg-card border-border rounded-t-3xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-5 mt-5 pb-8 page-px">
            <div>
