...
interface Friend extends Profile {
  rowId: string; // friends table row id
}

interface PendingRequest extends Profile {
  rowId: string;
...
const Friends = () => {
  ...
  // ── Fetch all friend rows for current user ────────────
  const fetchFriends = useCallback(async () => {
    if (!user) return;
    setLoading(true);

    // 1. Get every row where I am either side
    ...
    let profileMap: Record<string, string> = {}; // id → username
    ...
    // 2. Split into accepted friends and pending requests
    ...
    for (const row of friendRows) {
      ...
      if (row.status === 'accepted') {
        ...
        // inbound = someone sent me a request (friend_id = me)
        // outbound = I sent request (user_id = me)
        const direction: 'inbound' | 'outbound' =
          row.friend_id === user.id ? 'inbound' : 'outbound';
  ...
  const handleSearch = async () => {
    ...
    if (error || !data) {
      setSearchError(`No user found with username "${trimmed}"`);
      return;
    }
    ...
    const alreadyFriend = friends.some(f => f.id === data.id);
    const alreadyPending = pending.some(p => p.id === data.id);
    ...
    if (alreadyPending) {
      setSearchError('A friend request already exists with this user.');
      return;
    }
            ...
            <div className="mb-6">
              ...
              <div className="space-y-2">
                {inbound.map(req => (
                  ...
                  >
                    <div className="w-10 h-10 touch-min rounded-full bg-primary/10 flex items-center justify-center text-lg font-bold text-primary">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleAccept(req.rowId, req.username)}
              ...
              <div className="space-y-2">
                {outbound.map(req => (
                  <div
                    ...
                    className="glass-card p-4 flex items-center gap-3 animate-slide-up"
                  >
                    <div className="w-10 h-10 touch-min rounded-full bg-secondary flex items-center justify-center text-lg font-bold text-muted-foreground">
                      {req.username[0].toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-foreground">@{req.username}</p>
          ...
          {filteredFriends.length === 0 ? (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-sm mt-1">
                {!search && 'Tap + to add friends by username'}
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              {filteredFriends.map(friend => (
                ...
                >
                  ...
                  <button
                    onClick={() => handleUnfriend(friend.rowId, friend.username)}
